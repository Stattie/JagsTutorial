...
# Data
stm <- read.table("data/stm.txt",header=TRUE)
y <- stm$ce
...
data.list <- list(y=y, x = x, n.obs=n.obs)

post.runjags <- run.jags(model = "LogisticRegression", data = data.list, inits = inits.list,
